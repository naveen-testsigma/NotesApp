...
export class TodolistService {
  ...
  findAll(id : BigInt) : Observable<Todolist[]>{
    ...
    return this.http.get<Todolist[]>( `http://localhost:8080/todolist/search?query=id:${id}`,{headers:this.headers}).pipe(
  ...
  update(todoUpdate : Todolist) : Observable<Object>{
    ...
    return this.http.put<Todolist>(`http://localhost:8080/todolist/${todoUpdate.id}`,todoUpdate.serialize(),{headers:this.headers});
  }

  add(todoUpdate: Todolist) :Observable<Object>  {

    console.log("post here "+ todoUpdate.id + ' '+ todoUpdate.userId + ' '+todoUpdate.todoData);
   return  this.http.post(`http://localhost:8080/todolist/`,todoUpdate.serialize(),{headers:this.headers});
  }
  ...
  search(searcher : any) : Observable<Todolist[]> {
    ...
    {
      return this.http.get<Todolist[]>(`http://localhost:8080/todolist/search?query=id:${searcher.userId}`,{headers:this.headers}).pipe(
        map(data=> data.map(data1=> new Todolist().deserialize(data1))),
        catchError(() => throwError('Problem while fetching ElementFilter'))
      );
    }
      ...
      return this.http.get<Todolist[]>(`http://localhost:8080/todolist/search?query=id:${searcher.userId},title:${searcher.todoData}`, {headers: this.headers}).pipe(
        map(data => data.map(data1 => new Todolist().deserialize(data1))),
        catchError(() => throwError('Problem while fetching ElementFilter'))
      );
    }
